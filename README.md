...
<img  src=./image/bagging.png width="70%" height="70%">
 ...
 #### 결선(./final/kin/BiMPM_with_wikiVec/)
 
   결선 시작전 공지에서 유사도 문제 dataset의 구성이 봐뀌었다는 설명을 들었다. 기존까지의 데이터셋은 모든 주제 지식인 질문 데이터셋에서 일부분을 test set으로 빼고 나머지를 training set 으로 주는 방식이였다. 즉, training set과 test set에 사용된 단어들이 거의 중복 된다고 볼 수 있다. 그런데 결선 데이터셋은 이와 달리 training set 질문들의 주제와 test set의 질문 주제가 다르게 구성 했다는 설명을 들었다. 그래서 예선때와 결과가 많이 달라질 것이라고 설명을 들었다. 
   
   결선이 시작하고 준비했던 앙상블 모델을 적용해보았더니 60%도 안되는 정확도를 볼 수 있었다. 0과 1중 하나를 고르는 문제이기 때문에 60% 정확도는 의미가 없다고 할 수 있다. 이유를 생각해 본 결과 다음과 같다.
1. 우리 모델은 training set으로부터 단어들을 추출하고 이를 기반으로 학습한다.
2. 
