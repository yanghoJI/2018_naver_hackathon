...
def read_all_instances_kin(inpath, word_vocab=None, label_vocab=None, char_vocab=None, max_sent_length=100,
                       max_char_per_word=10, isLower=True, is_dev = False):
    if is_dev:
        data_path = os.path.join(inpath, 'test', 'kin_test_data_2.txt')
        label_path = os.path.join(inpath, 'test', 'kin_test_label_2.txt')
    else:
        data_path = os.path.join(inpath, 'train', 'train_data')
        label_path = os.path.join(inpath, 'train', 'train_label')
    instances = []
    #infile = open(inpath, 'rt', encoding = 'utf-8')
    data_infile_f = open(data_path, 'rt', encoding='utf-8')
    data_infile = data_infile_f.readlines()
    label_infile_f = open(label_path, 'rt', encoding='utf-8')
    label_infile = label_infile_f.readlines()
    if len(data_infile) != len(label_infile):
        print('data length unmatched')
    idx = -1
    for i in range(len(label_infile)):
        idx += 1
        line = data_infile[i].strip()
        if line.startswith('-'): continue
        items = re.split("\t", line)
        #items = line.split('    ')
        label = label_infile[i][0].strip()
        sentence1 = items[0].strip()
        sentence2 = items[1].strip()
        cur_ID = "{}".format(idx)
        if len(items)>=4: cur_ID = items[3]
        #if isLower:
        #    sentence1 = sentence1.lower()
        ...
        instances.append((label, sentence1, sentence2, label_id, word_idx_1, word_idx_2, char_matrix_idx_1, char_matrix_idx_2, cur_ID))
    data_infile_f.close()
    label_infile_f.close()
    return instances

def read_all_instances_kin_rawdata(rawdata, word_vocab=None, label_vocab=None, char_vocab=None, max_sent_length=100,
                       max_char_per_word=10, isLower=True, is_dev = False):
    data_infile = rawdata
    instances = []
    ...
    for i in range(len(data_infile)):
        ...
        instances.append((0, sentence1, sentence2, 0, word_idx_1, word_idx_2, char_matrix_idx_1, char_matrix_idx_2, cur_ID))


    if len(data_infile) != len(instances):
        raise notmatched
    return instances
...
class SentenceMatchDataStream(object):
    def __init__(self, inpath, word_vocab=None, char_vocab=None, label_vocab=None,
                 isShuffle=False, isLoop=False, isSort=False, options=None, is_dev=False, raw_data = False, bagging=None):
        if raw_data:
            instances = read_all_instances_kin_rawdata(inpath, word_vocab=word_vocab, label_vocab=label_vocab,
                                               char_vocab=char_vocab, max_sent_length=options.max_sent_length,
                                               max_char_per_word=options.max_char_per_word,
                                               isLower=True, is_dev=is_dev)
        else:
            instances = read_all_instances_kin(inpath, word_vocab=word_vocab, label_vocab=label_vocab,
        ...
        if bagging != None:
            print('Apply bagging... rate = {}'.format(bagging))
            num_bat = int(self.num_batch * bagging)
            self.index_array = self.index_array[:num_bat]
            self.num_batch = num_bat
            if len(self.index_array) !=self.num_batch:
