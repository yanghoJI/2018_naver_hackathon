...
class SentenceMatchModelGraph(object):
    ...
    def create_model_graph(self, num_classes, word_vocab=None, char_vocab=None, is_training=True, global_step=None):
        ...
        if word_vocab == []:
            word_vocab_triger = True
            word_vocab = 1
        else:
            word_vocab_triger = False
        ...
        if word_vocab is not None:
            ...
            with tf.device(cur_device):
                if word_vocab_triger == True:

                    self.word_embedding = tf.get_variable("word_embedding", [879129, options.word_emb_dim], trainable=word_vec_trainable,
                                                          dtype=tf.float32)
                    ##real = 127499   dummy = 211
                    ##real = 879129   dummy = 211

                    ##real = 86780   dummy = 211
                else:
                    ...
                    print('word emb size cheak = real = {}\tpredict = {}'.format(word_vocab.word_vecs.shape, [879129, options.word_emb_dim]))
            ...
            if word_vocab_triger == True:
                input_dim+= options.word_emb_dim
            else:
                input_dim += word_vocab.word_dim


        if char_vocab == []:
            char_vocab_triger = True
            char_vocab = 1
        else:
            char_vocab_triger = False
        ...
        if options.with_char and char_vocab is not None:
            ...
            char_dim = self.options.char_emb_dim

            #options.use_cudnn = True
            if char_vocab_triger ==True:
                self.char_embedding = tf.get_variable("char_embedding", [2757, char_dim], dtype=tf.float32)
                ##real = 2757   dummy = 224
                ##real = 2157   dummy = 224
            else:
                self.char_embedding = tf.get_variable("char_embedding", initializer=tf.constant(char_vocab.word_vecs), dtype=tf.float32)
                print('word emb size cheak = real = {}\tpredict = {}'.format(char_vocab.word_vecs.shape, [2757, char_dim]))
