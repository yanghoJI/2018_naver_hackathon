...
if __name__ == '__main__':
    ...
    parser.add_argument('--dropout_rate', type=float, default=0.25, help='Dropout ratio.')
    ...
    parser.add_argument('--word_emb_dim', type=int, default=300, help='word_emb_dim')
    ...
    parser.add_argument('--max_sent_length', type=int, default=15, help='Maximum number of words within each sentence.')
    ...
    parser.add_argument('--with_moving_average', default=True, help='with_moving_average', action='store_true')
    ...
    if config.mode == 'train':
        ...
        ##### load load wiki_vector #####
        print('load wiki_vec!!')
        wikitime = time.time()
        en_model = KeyedVectors.load_word2vec_format('/wiki.ko.vec')
        # Getting the tokens
        word2id = {}
        for word in en_model.vocab:
            word2id[word] = en_model.vocab[word].index
        print('wiki vec load time = {:.2f}'.format(time.time()- wikitime))
        word_vocab.word2id = word2id
        word_vocab.vocab_size = len(word2id)
        word_vocab.word_vecs = en_model.vectors
