    ...
    def create_model_graph(self, num_classes, word_vocab=None, char_vocab=None, is_training=True, global_step=None):
        ...
        if word_vocab is not None:
            ...
            with tf.device(cur_device):
                if word_vocab_triger == True:
                    ...
                    self.word_embedding = tf.get_variable("word_embedding", [86780, options.word_emb_dim], trainable=word_vec_trainable,
                                                          dtype=tf.float32)
                    ##real = 86774   dummy = 211
                    ...
                    print('word emb size cheak = real = {}\tpredict = {}'.format(word_vocab.word_vecs.shape, [86780, options.word_emb_dim]))
        ...
        if options.with_char and char_vocab is not None:
            ...
            if char_vocab_triger ==True:
                self.char_embedding = tf.get_variable("char_embedding", [2161, char_dim], dtype=tf.float32)
                ##real = 2161   dummy = 224
            else:
                self.char_embedding = tf.get_variable("char_embedding", initializer=tf.constant(char_vocab.word_vecs), dtype=tf.float32)
                print('word emb size cheak = real = {}\tpredict = {}'.format(char_vocab.word_vecs.shape, [2161, char_dim]))
