...
def my_lstm_layer(input_reps, lstm_dim, input_lengths=None, scope_name=None, reuse=False, is_training=True,
    ...
    with tf.variable_scope(scope_name, reuse=reuse):
        if use_cudnn:
            ...
            lstm = tf.contrib.cudnn_rnn.CudnnLSTM(1, lstm_dim, direction="bidirectional", name="{}_cudnn_bi_lstm".format(scope_name), dropout=dropout_rate if is_training else 0)
            #lstm = tf.contrib.cudnn_rnn.CudnnLSTM(1, lstm_dim, direction="bidirectional",
            #                                      input_size=inputs.shape,
            #                                     dropout=dropout_rate if is_training else 0)
            #lstm = tf.contrib.cudnn_rnn.CudnnLSTM(1, lstm_dim, direction="bidirectional", dropout=dropout_rate if is_training else 0)
            outputs, _ = lstm(inputs)
            outputs = tf.transpose(outputs, [1, 0, 2])
            f_rep = outputs[:, :, 0:lstm_dim]
            b_rep = outputs[:, :, lstm_dim:2*lstm_dim]
        else:
            print('cudnn not used')
