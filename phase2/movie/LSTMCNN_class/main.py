...
if __name__ == '__main__':
    ...
    def dev_step(x_batch, y_batch, writer=None):
        ...
        step, devloss, acc = sess.run([global_step, model.loss, model.accuracy], feed_dict)

        time_str = datetime.datetime.now().isoformat()
        print("validation step : {}: step {},\t validattion loss {:g},\t acc {:g}".format(time_str, step, devloss, acc))
        return devloss, acc
    ...
    if config.mode == 'train':
        ...
        for batch in batches:
            ...
            if current_step % config.checkpoint_every == 0:
                ...
                devtime = time.time()
                now_valloss, acc = dev_step(x_dev, y_dev, writer=None)
